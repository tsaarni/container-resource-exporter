...
var cli struct {
	...
	Mix        MixCmd        `cmd:"mix" help:"Churn on KV v2 at kv/ with configurable traffic mix."`
	BulkDelete BulkDeleteCmd `cmd:"bulk-delete" help:"Delete secrets from KV v1 (cleanup helper)."`
...
func main() {
	...
	switch ctx.Command() {
	case "kv-write":
		runKVWrite(addr, cli.KVWrite.Token, cli.KVWrite.Count, cli.KVWrite.RPS, cli.KVWrite.Concurrency, cli.KVWrite.Size)
	case "kv-read":
		runKVRead(addr, cli.KVRead.Token, cli.KVRead.Count, cli.KVRead.RPS, cli.KVRead.Concurrency, cli.KVRead.Pool)
	case "transit":
